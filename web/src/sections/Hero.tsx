import { profile, photo } from "../content/profile.js";
...
export default function Hero() {
  return (
    ...
    >
      <div className="grid items-center gap-12 lg:grid-cols-[1.5fr_0.9fr] lg:gap-14">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-7 text-balance">
            <span
              className="font-display block text-[2.5rem] font-medium leading-[1] tracking-tight text-paper animate-fade-up sm:text-6xl lg:text-[5.4rem]"
              style={{ animationDelay: "120ms" }}
            >
              {profile.name}
            </span>
            <span
              className="font-display mt-3 block text-[1.35rem] font-normal leading-tight text-paper/70 animate-fade-up sm:text-3xl lg:text-[2.5rem]"
          ...
          <p
            className="mt-7 max-w-xl text-balance text-[1.05rem] leading-relaxed text-paper-muted animate-fade-up sm:text-[1.2rem]"
            style={{ animationDelay: "320ms" }}
          >
            {profile.tagline}
          </p>

          {/* Scope-over-title frame */}
          <p
            className="mt-6 font-mono text-[0.72rem] uppercase leading-relaxed tracking-[0.1em] text-paper-faint animate-fade-up"
        ...
        {/* Portrait medallion: real photo when provided, refined monogram until then */}
        <div className="hidden animate-fade-up justify-self-center lg:block" style={{ animationDelay: "360ms" }}>
          <div className="relative aspect-square w-64 overflow-hidden rounded-2xl border border-white/[0.1] bg-ink-900/50 xl:w-72">
            <div className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-accent/15 blur-3xl" aria-hidden />
            {photo.available ? (
              <img
                src={photo.path}
                alt="Shay Kopilevich"
              ...
              <div className="flex h-full w-full flex-col items-center justify-center gap-3">
                ...
                <span className="font-mono text-[0.56rem] uppercase tracking-[0.2em] text-paper-faint">
                  Hod HaSharon, IL
                </span>
              </div>
            )}
